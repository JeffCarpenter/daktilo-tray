...
:PROPERTIES:
:owner: Codex
:last_reviewed: 2025-11-27T21:00:00-06:00
...
- ?? *Built-in reporting*: add a reusable PowerShell helper that turns `cargo llvm-cov --json` + test logs into a Markdown block appended to `$GITHUB_STEP_SUMMARY`, staying under the documented per-step size limits while surfacing pass/fail counts and coverage deltas inline.
- ?? *JUnit plumbing*: pilot `dorny/test-reporter@v2` (or CTRF) on Windows runs so PRs get rich test status checks without third-party dashboards; confirm artifact permissions for forked PRs and document the limitations + required workflow separation.
** Coverage Roadmap
:PROPERTIES:
:status: planning
