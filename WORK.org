...
:PROPERTIES:
:owner: Codex
:last_reviewed: 2025-11-27T23:50:00-06:00
...
- [ ] *Release workflow parity*: reconcile `.github/workflows/release.yml` with cargo-dist's generated template so `dist build --installer msi` no longer needs `--allow-dirty` while preserving our cargo-pants + signtool + smoke jobs.
** Coverage Roadmap
:PROPERTIES:
:status: in_review
...
- [ ] *Coverage deltas in PR comments*: follow-up to mirror the Pages URL + key deltas into PR comments for better discoverability.
