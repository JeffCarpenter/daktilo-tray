...
- [ ] Ensure the release workflow checks out the sibling `daktilo` sources (submodule checkout, extra `actions/checkout`, or vendored crate) so `cargo metadata` can resolve `daktilo_lib` during `dist plan`.
** Coverage Roadmap
:PROPERTIES:
:status: in_review
...
** 2025-11-27T20:03:37-08:00 Execution breakdown
- Step 1 (Release workflow dry-run): fetch template hash, trigger gh workflow run Release --ref main, watch logs, archive results, note in WORK.org.
- Step 2 (Installer report enhancements): add file path + timestamps in scripts/test-installer.ps1, update workflow summary, validate JSON output locally, log findings.
- Step 3 (Docs): extend HACKING.org with signtool reporting section, include schema + troubleshooting tips, cross-link release automation chapter.


** 2025-11-27T20:13:11-08:00 Release workflow dry-run attempt
- gh workflow run Release --ref main failed because GitHub reports no workflow named Release on origin/main; remote repo lacks the refreshed workflow until changes are pushed.
- gh workflow run release.yml also returned HTTP 404 for ndtoan96/daktilo-tray, so fully exercising CI is blocked locally; will proceed with installer-report + docs steps after capturing template metadata.
** 2025-11-28T12:25:00-06:00 Release workflow dry-run attempt #2
- Re-ran `gh workflow run Release --ref main`; GitHub CLI still reports "could not find any workflows named Release" because origin/main has not received the regenerated workflow yet.
- Queried GitHub via MCP `repo://ndtoan96/daktilo-tray/.../.github/workflows/release.yml`; API returned 404 confirming the workflow file is absent upstream, so CI dry-run remains blocked until we push.
- Action: proceed with installer-report + documentation improvements locally, note that remote validation will remain pending until workflow lands on origin/main.
** 2025-11-28T12:45:00-06:00 Installer report enrichment
- Extended `scripts/test-installer.ps1` to capture the MSI path plus signtool start/end timestamps, duration, command string, and signature status inside `signtool-report.json`.
- Updated `.github/workflows/release.yml` summary step to surface the new metadata (artifact path, verification timestamps, duration, log path) inside `$GITHUB_STEP_SUMMARY`.
- Documented the JSON schema + workflow consumption path in `HACKING.org` so contributors know how to debug CI failures.
** 2025-11-28T12:55:00-06:00 Build validation
- Ran `cargo build`; workspace succeeded with the usual upstream `daktilo_lib` warnings about `audio-tests` cfg + missing docs (unchanged from previous runs).
** 2025-11-28T13:05:00-06:00 Next steps checkpoint
- Blocked on remote workflow dry-run until `.github/workflows/release.yml` lands on origin/main; once pushed, re-run `gh workflow run Release --ref main` to validate cargo-dist template + smoke hooks.
- After the workflow exists upstream, continue with the installer-report verification (already done) and documentation updates, then proceed to remaining WORK.org roadmap items (installer report consumption wiring already done; next: coverage follow-ups once reprioritized).
** 2025-11-28T12:05:00-08:00 Fork + workflow dispatch dry-run
- Forked `ndtoan96/daktilo-tray` into `JeffCarpenter/daktilo-tray`, added it as `jeff`, and pushed `main` so we can exercise CI without origin perms.
- Added `workflow_dispatch` trigger to `.github/workflows/release.yml`, committed (`Enable release workflow dispatch`), and re-ran `gh workflow run Release --ref main`; dispatch now succeeds.
- Plan job fails because `cargo metadata` cannot load `daktilo_lib` at `../daktilo/crates/daktilo_lib` on GitHub runners—the sibling `daktilo` repo is missing, so the next step is to fetch it (submodule, sparse checkout, or artifact) before `dist plan`.
- Installer-smoke, build, host, and announce jobs all skipped because plan failed; need to unblock metadata resolution before re-running.
** 2025-11-28T12:25:00-06:00 Build verification
- Ran `cargo build`; workspace still compiles cleanly with the known `daktilo_lib` warnings (`audio-tests` cfg flag + missing docs). Confirms local tree is consistent before tweaking CI dependencies.

** 2025-11-28T13:32:00-06:00 Release workflow dependency bootstrap plan
- Recap blocker: `dist plan` on GitHub runners fails to resolve the path dependency `../daktilo/crates/daktilo_lib`.
- Decided to keep the path dependency (so local contributors can keep hacking against `../daktilo`) and instead teach CI to clone the sibling repo on demand; evaluated swapping to crates.io `daktilo_lib` but keeping the path avoids surprises while we still maintain patches out of tree.
- Action breakdown:
  1. Introduce workflow-level env knobs for the dependency repo/ref so we can change forks/channels without editing every job.
  2. After each `actions/checkout`, run a light-weight clone step that materializes `../daktilo` (with `--depth 1 --branch main`) on both Linux and Windows runners.
  3. Re-run `gh workflow run Release --ref main` once pushed to ensure the plan job clears the metadata phase before resuming installer/doc follow-ups already logged in WORK.org.

** 2025-11-28T13:45:00-06:00 Release workflow + AGENTS updates
- Added global `DAKTILO_REPO/DAKTILO_REF` env vars plus bootstrap steps in the plan/build jobs so CI clones `ndtoan96/daktilo` into `../daktilo` before invoking `dist`.
- Updated `AGENTS.md` “Additional References” with the UTF-8 reminder so future scripting uses the right encoding instead of tripping the Windows `UnicodeDecodeError` we kept hitting.
- Next: validate the refreshed workflow via `gh workflow run Release --ref main` after pushing to the fork, then log the results and continue with installer-report/doc tasks from WORK.org:245+.

** 2025-11-28T13:52:00-06:00 Build verification
- Ran `cargo build`; workspace still compiles successfully with the usual upstream `daktilo_lib` warnings (`audio-tests` cfg + missing docs). Confirms the workflow/AGENTS edits didn’t break local builds before triggering CI.

** 2025-11-28T14:00:00-06:00 Release dry-run + fix
- Triggered `gh workflow run Release --ref main` (run `19754876933` on the fork); plan job now starts but fails early because the manual `git clone https://github.com/${DAKTILO_REPO}` step prompts for credentials on hosted runners.
- Replaced the ad-hoc clone steps with additional `actions/checkout@v4` invocations (with `path: ../daktilo`) across the plan/build/smoke/global jobs so GitHub injects the right token automatically.
- Next: push the update, re-run the workflow, and capture whether `dist plan` can finally load `daktilo_lib`; continue with the remaining installer/report/doc tasks afterwards.

** 2025-11-28T14:10:00-06:00 Release dry-run attempt #4
- Reran `gh workflow run Release --ref main` (run `19754914879`); `actions/checkout` rejected `path: ../daktilo` with “Repository path … is not under …/daktilo-tray”.
- Updated the workflow again to clone into `daktilo-src` inside the workspace and explicitly move it to `../daktilo` (with OS-specific `mv`/`Move-Item` steps so both Linux and Windows runners end up with the sibling tree).
- Next up: push the fix, dispatch the workflow once more, and verify whether plan/build jobs finally reach `dist plan`.

** 2025-11-28T14:18:00-06:00 Release dry-run attempt #5
- Dispatch `gh workflow run Release --ref main` (run `19754951189`); plan job now fails because `actions/checkout` can’t find `ndtoan96/daktilo`.
- Realized the dependency repo we control is `JeffCarpenter/daktilo` (the upstream path doesn’t exist). Updated `DAKTILO_REPO` in the workflow env block accordingly so hosted runners pull from the same forked sources we use locally.
- Next: push the env tweak, trigger the workflow again, and confirm whether `dist plan` finally passes.

** 2025-11-28T14:27:00-06:00 Release dry-run attempt #6
- Run `gh workflow run Release --ref main` (run `19754978954`); dependency checkout + promote now works, but `dist host --tag=main` fails because workflow_dispatch on a branch sets `github.ref_name=main`, which isn’t a SemVer tag.
- Patched the plan-job expression to only invoke `dist host --steps=create --tag=…` when the ref actually points at a tag (`startsWith(github.ref, 'refs/tags/')`); otherwise the job falls back to `dist plan` for dry-runs.
- Next: push the guard, rerun the workflow, and capture whether the plan job clears so we can move on to installer/doc tasks.

** 2025-11-28T14:34:00-06:00 Release dry-run attempt #7
- Triggered `gh workflow run Release --ref main` (run `19755015321`). plan job now reaches `dist plan`, but cargo-dist exits 255 complaining that `.github/workflows/release.yml` diverged from the template (the diff against dist’s generated content lists our daktilo checkout + smoke wiring).
- Need to teach CI to set `DIST_ALLOW_DIRTY=1` for plan/build (or adopt `dist init --yes` + template overlay approach) so cargo-dist stops erroring on intentional customizations.
- Also noted the GitHub run log now stretches to ~567 lines; capture a follow-up to restructure jobs or emit structured logs so future debugging isn’t as noisy.
** 2025-11-27T21:42:37.127410-08:00 Dist workflow unblock for customizations
- Added DIST_ALLOW_DIRTY=1 to plan/build/global/host jobs in .github/workflows/release.yml so cargo-dist tolerates our daktilo checkout + smoke wiring.
- Rationale: release workflow intentionally diverges from template to clone daktilo sibling and run cargo-pants/smoke; allowing dirty ensures plan/build/host succeed while we keep automation intact.
- Next: re-dispatch gh workflow run Release --ref main to verify plan passes before moving on to installer-report + documentation tasks.
** 2025-11-27T21:51:01.273544-08:00 Cargo-dist template options
- Read cargo-dist book sections on config and CI customization to catalog supported extension hooks (plan/build/global custom jobs, github-build-setup, github-custom-runners).
- Conclusion: to drop allow-dirty we need to re-express our daktilo checkout + signtool/cargo-pants wiring via supported hooks: e.g., move daktilo fetch into a reusable workflow invoked via plan-jobs/local-artifacts-jobs, inject long setup steps via github-build-setup, and stash signtool summary ingestion in a custom local-artifacts job instead of hand-editing release.yml.
- Next: draft concrete plan (overlay files + dist config changes) so release.yml can be fully generated again, then test by running dist init --yes and confirming CI stays clean without allow-dirty.
*** Plan: cargo-dist template reconciliation
- [ ] Author reusable workflow .github/workflows/daktilo-checkout.yml that clones JeffCarpenter/daktilo into ../daktilo, outputs ready path, and expose it via plan/local-artifacts/global/installer jobs.
- [ ] Add plan-jobs/local-artifacts-jobs/global-artifacts-jobs entries in dist-workspace.toml so generated release.yml calls those reusable helpers instead of inline edits.
- [ ] Convert existing cargo-pants/signtool/test-installer wiring into either github-build-setup (for extra steps prior to dist build) or separate reusable workflows triggered via local-artifacts-jobs, keeping scripts in repo but off template.
- [ ] Run dist init --yes to regenerate release.yml from template; verify git diff only includes new reusable workflows + config, so allow-dirty can be removed.
** 2025-11-27T21:52:00.991896-08:00 Template reconciliation plan logged
- Research confirmed cargo-dist supports reusable workflows (`plan-jobs`, `local-artifacts-jobs`, etc.) plus github-build-setup/custom runners, so we can move daktilo checkout + signtool/pants wiring out of release.yml edits.
- Logged next steps: author helper workflows, add plan/local/global job references, convert setup into github-build-setup hook, rerun dist init --yes to regen clean workflow and drop allow-dirty.
- Next: implement the reusable workflow + config changes, regenerate release.yml, then re-trigger gh workflow run Release --ref main.
** 2025-11-27T21:56:59.335036-08:00 Next steps after allow-dirty unblock
- [ ] Author .github/workflows/daktilo-checkout.yml reusable workflow and wire it via dist plan/local/global job hooks so release.yml stays template-owned.
- [ ] Move cargo-pants + signtool setup into github-build-setup (or reusable workflows) and update dist-workspace.toml accordingly.
- [ ] Run dist init --yes to regenerate release.yml + Wix template, confirm clean git status, then drop allow-dirty.
- [ ] Dispatch gh workflow run Release --ref main to verify plan/build/smoke pass with the regenerated template.
*** 2025-11-28T14:45:00-06:00 Template hook implementation plan
- Reconfirm cargo-dist hook surface (plan/local/global jobs + github-build-setup) and sketch how daktilo checkout + cargo-pants/signing/test-installer map onto them.
- Define deliverables: reusable workflow for checkout, build-setup script hook, dist-workspace job arrays, regenerated release.yml without allow-dirty, and follow-up dry-run once pushed.
- Queue execution steps: (1) author reusable workflow + github-build-setup entry point, (2) update dist-workspace metadata to call them, (3) run dist init --yes to refresh CI + Wix artifacts, (4) validate locally (cargo build, release script dry-run) before the next dispatch attempt.
** 2025-11-28T15:05:00-06:00 Template hooks implemented
- Added endor/daktilo as a submodule and pointed Cargo.toml at the vendored crate so CI no longer needs to clone a sibling repo; refreshed Cargo.lock after the path change.
- Authored .github/build-setup.yml plus scripts/ci/verify-daktilo.ps1 so every build matrix run verifies the submodule state before calling dist.
- Added reusable workflows: supply-chain-gate.yml now runs cargo-pants as a plan-job gate, and windows-sign-smoke.yml handles signing + installer verification as a global-artifacts job (overwriting the Windows artifact after signtool/log collection).
- Updated dist-workspace.toml with github-build-setup, plan-jobs, and global-artifacts-jobs, then reran dist init --yes so .github/workflows/release.yml and wix/main.wxs match the upstream template without allow-dirty hacks.
- Release workflow diff is now limited to the autogenerated hooks; next step is to run gh workflow run Release --ref main once the fork gets this commit so we can confirm plan/build pass with the reusable jobs.
** 2025-11-28T15:25:00-06:00 Build verification
- Ran cargo build; workspace compiles (warnings limited to daktilo_lib docs/cfg hints inherited from upstream). Confirmed vendored crate + regenerated CI hooks keep the tree buildable.
