...
** Task Breakdown
- [ ] Parse `dist-workspace.toml` for `[workspace.metadata.dist.autostart]` defaults (fallback to safe values if missing).
- [ ] Create `AutostartController` abstraction using `auto-launch` (only instantiate on supported OS) that exposes `current()` and `set(bool)`.
- [ ] Extend cached `State` with `run_on_login` and serialize.
- [ ] Add tray checkbox + event handling that syncs UI, `State`, controller errors, and logs.
- [ ] Persist updated state on exit along with new field.
- [ ] Update Cargo manifest (dependencies + metadata) and add `dist-workspace.toml` capturing release knobs + human-friendly copy.
- [ ] Document feature in README (usage + how to set default via dist config) and recap changes here once done.
