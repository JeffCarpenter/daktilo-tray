...
- `.github/workflows/coverage.yml` executes the same helper on every PR + `main` push, uploads the coverage folder as an artifact, and streams a Markdown snapshot into `$GITHUB_STEP_SUMMARY` so you can review headline numbers without downloading artifacts. Keep each summary under the 1 MiB GitHub limit by sticking to the generated table.
