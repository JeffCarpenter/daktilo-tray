...
- `.github/workflows/coverage.yml` calls the same helper on `windows-latest`, feeds the generated JUnit file into `dorny/test-reporter@v2` for PR annotations, appends the Markdown summary to `$GITHUB_STEP_SUMMARY`, and uploads the combined artifact so you can drill into LCOV/HTML/JUnit locally. Keep the summary lean so it stays under GitHub's 1 MiB per-step limit.
