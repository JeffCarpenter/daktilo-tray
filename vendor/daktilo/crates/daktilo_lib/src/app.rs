...
impl App {
    ...
    // TODO: Return a simple effect or toggle sounds (more or less) directly from here
    /// Handle the key events.
    pub fn handle_key_event(&mut self, event: Event) -> Result<Msg> {
        match event.event_type {
            EventType::KeyPress(key) | EventType::KeyRelease(key) => {
                ...
                if key == self.mute_key && event_type == KeyEvent::KeyRelease {
                    self.toggle_mute()
                }
                if (!self.mute) && event_type == KeyEvent::KeyRelease {
                    let key_config = self
                        .preset
                        .key_config
                        .clone()
